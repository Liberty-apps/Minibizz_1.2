/*
  # Système d'abonnements et sites vitrines

  1. Nouvelles Tables
    - `plans` - Plans tarifaires avec fonctionnalités et limites
    - `abonnements` - Abonnements utilisateurs avec gestion Stripe
    - `sites_vitrines` - Sites vitrines des utilisateurs
    - `templates_sites` - Templates disponibles pour les sites
    - `limites_utilisation` - Suivi des quotas d'utilisation
    - `historique_paiements` - Historique des transactions
    - `pages_sites` - Pages des sites vitrines

  2. Sécurité
    - RLS activé sur toutes les tables
    - Politiques d'accès appropriées pour chaque table
    - Fonctions de vérification des limites d'utilisation

  3. Fonctionnalités
    - Plans avec limites configurables
    - Gestion des abonnements avec Stripe
    - Création de sites vitrines
    - Suivi des quotas d'utilisation
*/
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'plans' AND policyname = 'Anyone can read active plans'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'abonnements' AND policyname = 'Users can read own subscription'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'abonnements' AND policyname = 'Users can insert own subscription'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'abonnements' AND policyname = 'Users can update own subscription'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'sites_vitrines' AND policyname = 'Users can manage own sites'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'templates_sites' AND policyname = 'Anyone can read active templates'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'limites_utilisation' AND policyname = 'Users can read own usage limits'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'limites_utilisation' AND policyname = 'System can manage usage limits'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'historique_paiements' AND policyname = 'Users can read own payment history'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'pages_sites' AND policyname = 'Users can manage own site pages'
  ) THEN
...
-- Ajouter la référence de template_id vers templates_sites
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'sites_vitrines_template_id_fkey'
  ) THEN
    ALTER TABLE sites_vitrines 
    ADD CONSTRAINT sites_vitrines_template_id_fkey 
    FOREIGN KEY (template_id) REFERENCES templates_sites(id);
  END IF;
END $$;

-- Triggers pour updated_at (seulement si ils n'existent pas)
DO $$ BEGIN
  IF NOT EXISTS (
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'update_abonnements_updated_at'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'update_sites_vitrines_updated_at'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'update_limites_utilisation_updated_at'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'update_pages_sites_updated_at'
  ) THEN
...
-- Insertion des plans par défaut (seulement si ils n'existent pas)
...
) AS new_plans(nom, prix_mensuel, prix_annuel, description, fonctionnalites, limites, couleur, ordre)
WHERE NOT EXISTS (SELECT 1 FROM plans WHERE plans.nom = new_plans.nom);

-- Insertion des templates par défaut (seulement si ils n'existent pas)
...
) AS new_templates(nom, description, type_template, structure, styles_defaut)
WHERE NOT EXISTS (SELECT 1 FROM templates_sites WHERE templates_sites.nom = new_templates.nom);
