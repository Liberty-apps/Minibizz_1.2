/*
  # Fix for Stripe and Subscription Schema

  1. New Tables
    - `plans` - Subscription plans with pricing and features
    - `abonnements` - User subscriptions
    - `sites_vitrines` - Website builder sites
    - `templates_sites` - Website templates
    - `limites_utilisation` - Usage limits tracking
    - `historique_paiements` - Payment history
    - `pages_sites` - Website pages

  2. Security
    - Enable RLS on all tables
    - Add policies for secure access
    - Create functions for usage limits
...
DROP POLICY IF EXISTS "Users can read own subscription" ON abonnements;
DROP POLICY IF EXISTS "Users can insert own subscription" ON abonnements;
DROP POLICY IF EXISTS "Users can update own subscription" ON abonnements;
DROP POLICY IF EXISTS "Users can manage own sites" ON sites_vitrines;
DROP POLICY IF EXISTS "Anyone can read active templates" ON templates_sites;
DROP POLICY IF EXISTS "Users can read own usage limits" ON limites_utilisation;
DROP POLICY IF EXISTS "System can manage usage limits" ON limites_utilisation;
DROP POLICY IF EXISTS "Users can read own payment history" ON historique_paiements;
DROP POLICY IF EXISTS "Users can manage own site pages" ON pages_sites;
...
-- Triggers pour updated_at (seulement si la fonction existe)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'update_updated_at_column') THEN
    ...
    DROP TRIGGER IF EXISTS update_sites_vitrines_updated_at ON sites_vitrines;
    ...
    DROP TRIGGER IF EXISTS update_limites_utilisation_updated_at ON limites_utilisation;
    ...
    DROP TRIGGER IF EXISTS update_pages_sites_updated_at ON pages_sites;
...
CREATE OR REPLACE FUNCTION check_usage_limit(p_user_id uuid, p_limit_type text, p_increment integer DEFAULT 1) 
RETURNS boolean 
AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION increment_usage(p_user_id uuid, p_limit_type text, p_increment integer DEFAULT 1) 
RETURNS void 
AS $$
DECLARE
  current_period_start date;
